...
# 1. Remove the broken cards
html = re.sub(r'<article class="card concept-card reveal"[^>]*data-tag="[CEG]-new".*?</article>', '', html, flags=re.DOTALL)
html = re.sub(r'<article class="card concept-card reveal"[^>]*data-title="DIKW 피라미드와 Verbert 학습 분석 모델".*?</article>', '', html, flags=re.DOTALL)
html = re.sub(r'<article class="card concept-card reveal"[^>]*data-title="탐구 질문 \(Inquiry Question\) 설계".*?</article>', '', html, flags=re.DOTALL)
html = re.sub(r'<article class="card concept-card reveal"[^>]*data-title="성취도 × 참여도 매트릭스".*?</article>', '', html, flags=re.DOTALL)
html = re.sub(r'<article class="card concept-card reveal"[^>]*data-title="진단 결과 3단계 프레임워크".*?</article>', '', html, flags=re.DOTALL)
...
for c in cards_to_add:
    ...
    if cat_cards:
        ...
        escaped_def = html_lib.escape(c['def'])
        text_content = f"{cat} {c['title']} {re.sub('<[^<]+>', '', c['def'])}"
        
        # Build new card
        new_card = f"""
      <article class="card concept-card reveal" data-cat="{cat}" data-text="{text_content}" data-color="{color}" data-tag="{cat}-new" data-title="{c['title']}" data-def="{escaped_def}" data-insight="{c['insight']}" data-apply="{c['apply']}" data-src="{c['src']}">
        <div class="inner">
...
for cat in ["C", "E", "G"]:
    ...
    for i, card_html in enumerate(cards):
        ...
        new_card_html = re.sub(r'<span class="tag"[^>]*>[^<]+</span>', f'<span class="tag" style="background:rgba({get_rgb(colors[cat])[0]},{get_rgb(colors[cat])[1]},{get_rgb(colors[cat])[2]},0.12);color:{colors[cat]}">{cat}{i+1}</span>', new_card_html)
        html = html.replace(card_html, new_card_html)

with codecs.open('인공지능활용_선도교사연수_초등_핵심개념_웹사이트.html', 'w', 'utf-8') as f:
    f.write(html)

print("Replaced 4 new cards.")
